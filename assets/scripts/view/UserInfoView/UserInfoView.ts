...
export default class UserInfoView extends BaseUI {
        ...
        App.ui.open(UIConstant.AllAlertView, null,
            {
                data: {
                    showKey: 'otherAward',
                    msg: `是否确定消耗<color=#FA682C>${UIUtil.toStone(this.userInfo.nextLevel.props)}</color>水果\n将宠物升级至LV${this.userInfo.nextLevel.level}？\n本次升级预计获赠<color=#FA682C>${UIUtil.toStone(this.userInfo.nextLevel.reward)}</color>礼券`,
                    okCallBack: () => {
                        App.PetProxy.PetLevelUp({
                            onSuccess: (res) => {
                                App.audio.playEffectByName('open_treasure_boom')
    ...
    protected openUpgrade1() {
        ...
        const msg = this.userInfo.realMaxLevel >= this.userInfo.maxLevel ?
            `是否确定消耗<color=#FA682C>${UIUtil.toStone(this.userInfo.maxLevelNeedProps)}</color>水果\n将宠物升级至LV${this.userInfo.maxLevel}？\n
            本次升级预计获赠<color=#FA682C>${UIUtil.toStone(this.userInfo.maxLevelReward)}</color>礼券。`
            :
            `<color=#FA682C>今日喂食上限最高可升至LV${this.userInfo.realMaxLevel}</color>\n是否确定消耗<color=#FA682C>${UIUtil.toStone(this.userInfo.realMaxLevelNeedProps)}</color>水果\n将宠物升级至LV${this.userInfo.realMaxLevel}？\n本次升级预计获赠<color=#FA682C>${UIUtil.toStone(this.userInfo.realMaxLevelReward)}</color>礼券`
        if (this.userInfo.petInfo.level >= this.userInfo.realMaxLevel) {
            App.PetProxy.PetLevelUp({
                onSuccess: (res) => {
                    App.audio.playEffectByName('open_treasure_boom')
