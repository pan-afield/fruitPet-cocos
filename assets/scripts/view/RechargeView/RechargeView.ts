                        ...
                        onSuccess: (res) => {
                            if (res.ShowIphoneCharge) {
                                ...
                                wx.openCustomerServiceConversation({
                                    ...
                                    sendMessageTitle: '点击客服回复链接进行充值',
                                    sendMessagePath: `goodid=${this.currentObj.rid}`,//返回游戏带参
                            ...
                            if (res.Result === -5) {
                                if (Config.gameFlag) {
                                    ...
                                    wx.requestMidasPayment({
                                        ...
                                        currencyType: 'CNY',
                                        platform: 'android',
                                        buyQuantity: this.currentObj.price,//购买数量
                                        success: (res) => {
                                            console.log(res);
                                            App.GoodsProxy.userPay({
                                                onSuccess: (res) => {
                ...
                this.gooodsList.forEach((items: any, indexs) => {
                    if (index === indexs) {
                        ...
                        if (items.coin !== items.realcoin) {
                            this.rewardCoin.getComponent(cc.Label).string = `[首充]${UIUtil.toStone(items.realcoin)}金币`
                        } else {
                            this.rewardCoin.getComponent(cc.Label).string = `${UIUtil.toStone(items.realcoin)}金币`
                        }
                        ...
                        this.another.getComponent(cc.Label).string = `VIP经验+${items.credits}`
    ...
    protected getGoodsList() {
        let onSuccess = (res) => {
            ...
            res.list.forEach((item: any, index) => {
                ...
                vipExp.getComponent(cc.Label).string = `小鱼干+${item.credits}`
