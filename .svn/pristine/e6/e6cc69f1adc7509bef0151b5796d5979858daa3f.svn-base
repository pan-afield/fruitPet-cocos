...
export default class ConvertView extends BaseUI {
    ...
    private chooseGoods(event) {
        ...
        if (node.data.status === -2) {
            ...
            App.ui.open(UIConstant.MallTipView, null, {
                data: { msg: `该商品VIP${node.data.viplevel}及以上可兑换，前往充值可快速提升VIP等级！` },
                okCallBack: () => {
                ...
                if (this.confrimType === 'wood') {
                    ...
                    this.goodsList['card'].forEach((items, indexs) => {
                        if (index === indexs) {
                            ...
                            if (items.name.includes('京东卡')) {
                                this.hint.getComponent(cc.Label).string = '请重复确认手机号！卡号和卡密将以短信形式发放！'
                            } else if (items.name.includes('爱奇艺')) {
                                this.hint.getComponent(cc.Label).string = '请重复确认手机号！爱奇艺月卡将直接充至对应会员账号！'
                            } else if (items.name.includes('优酷')) {
                                this.hint.getComponent(cc.Label).string = '请重复确认手机号！视频卡的卡密将以短信形式发放！'
                            }
                    ...
                    this.goodsList['phone'].forEach((items, indexs) => {
                        if (index === indexs) {
                            ...
                            this.phoneNum.active = true
                            this.hint.getComponent(cc.Label).string = '*请重复确认手机号！话费将在2小时内到账！'
                            this.hint.active = true
                            this.addressInfo.active = false
                            this.chooseAddress.active = false
                            this.woodName.active = false
        ...
        if (this.addressInfo.active) {
            ...
            let onSuccess = (res) => {
                ...
                if (res.Data === 1 || res.Data === 2) {
                    App.ui.open(UIConstant.MallTipView, null, {
                        data: { msg: res.Msg },
                        okLabel: res.Data === 2 ? '前往充值' : App.gdata.openShop ? '前往充值' : '确定',
                        okCallBack: () => {
                            this.close()
                            App.ui.open(UIConstant.RechargeView)
                        }
